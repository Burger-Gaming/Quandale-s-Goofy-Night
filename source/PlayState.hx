...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'quandale': //hey guys quandale dingle here
				var bg:BGSprite = new BGSprite('stage/sky', -475.45, -108.45);
				bg.antialiasing = ClientPrefs.globalAntialiasing;
				add(bg);

				var clouds:BGSprite = new BGSprite('stage/clouds', -216.25, -115.9);
				clouds.antialiasing = ClientPrefs.globalAntialiasing;
				add(clouds);

				var trees:BGSprite = new BGSprite('stage/trees', -489.55, -76.9);
				trees.antialiasing = ClientPrefs.globalAntialiasing;
				add(trees);

				var grass:BGSprite = new BGSprite('stage/grass', -420.45, 463.7);
		...
		var fadeIn:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
		fadeIn.cameras = [camHUD];
		fadeIn.scrollFactor.set();
		add(fadeIn);
		if (SONG.song != 'Tarnation') fadeIn.visible = false;
		new FlxTimer().start(1, function(tmr:FlxTimer){
			FlxTween.tween(fadeIn, { alpha: 0 }, 4, { onComplete: function(twn:FlxTween){
				fadeIn.destroy();
			}});
		});
	...
	function startCharacterPos(char:Character, ?gfCheck:Bool = false) {
		...
		if (char == boyfriend && curStage != 'quandale'){
	...
	override function beatHit()
	{
		...
		if (SONG.song == 'Tarnation' && camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms && curBeat >= 4 && curBeat % 1 == 0){
			camGame.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
